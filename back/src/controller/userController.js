...
exports.signup = async function(req,res){
    ...
    const isVaildEmail =  /^[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/i;
    ...
    const isValidPassword = /^(?=.*\d)(?=.*[a-zA-Z])[0-9a-zA-Z]{8,20}$/; // 8 ~ 10자 영문, 숫자 조합
    if(!isValidPassword.test(password)){
        return res.send({
            isSuccess: false,
            code: 400,
            message: "비밀번호 형식을 확인해주세요\n비밀번호는 8 ~ 20자 영문 혹은 숫자로 조합합니다.",
        });
    }
...
exports.getNicknameByToken = async function(req,res){
    ...
    const [userInfo] = await userDao.selectNicknameByTokenByuserIdx(userIdx); 
